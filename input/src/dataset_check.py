...
def compare_roberta_emb(file_1, file_2, emb_no=0):
    ...
    roberta_sim = 1 - cosine(emb_1, emb_2)
    print(f"cosine similarity roberta emb: {roberta_sim:.2f}")


def compare_cnn_emb(emb_idx_1, emb_idx_2, emb_no=0):
    embeddings = np.array(
        pickle.load(
            open("../data/birds/train/char-CNN-RNN-embeddings.pickle", "rb"),
            encoding="latin1",
        )
    )
...
def compare_embedding_quality(emb_idx_1=0, emb_idx_2=1, emb_no=0):
    ...
    text2 = (
        open(os.path.join(config.ANNOTATIONS, file_2 + ".txt"), "r")
        .read()
        .split("\n")[:-1]
    )
...
if __name__ == "__main__":
    ...
    print("__" * 80)
    check_model(
        file_idx=14, model="../../old_outputs/output-3/model/netG_epoch_110.pth"
    )
